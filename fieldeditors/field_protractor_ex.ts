...
export interface FieldProtractorExOptions extends pxtblockly.FieldProtractorOptions {
    sliderMinValue: number
    sliderMaxValue: number
    sliderInitValue: number
}

export class FieldProtractorEx extends Blockly.FieldSlider implements Blockly.FieldCustom {
        ...
        private reporterSub: SVGTextElement;
         ...
         * @constructor
         */
        constructor(value_: any, params: FieldProtractorExOptions, opt_validator?: Function) {
            super(String(params.sliderInitValue), params.sliderMinValue, params.sliderMaxValue, '1', '15', lf("Angle"), opt_validator);
            this.params = params;
        }
        ...
        createLabelDom_(labelText: string) {
            ...
            this.reporterSub = pxsim.svg.child(this.circleSVG, "text", {
                'x': 100, 'y': 140,
                'text-anchor': 'middle', 'dominant-baseline': 'middle',
                'style': 'font-size: 50px',
                'class': 'sim-text inverted number'
        ...
        setReadout_(readout: Element, value: string) {
            ...
            var resolutionValue =  parseInt(value) * 4096 / 360;
            resolutionValue = Math.round(resolutionValue) + 2048;
            this.reporterSub.textContent = resolutionValue.toString();
        }
        ...
        private updateAngle(angle: number) {
            if(angle > 0)
                ...
                angle = Math.min(0, Math.min(360, angle));
            const radius = 90;
            const pct = (360 - angle) / 180 * 3.141592 * radius;
